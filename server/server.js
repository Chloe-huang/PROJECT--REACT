...
app.post('/getCollections', function (req, res) {
    ...
    // console.log(req.body,'sdadafffffffffffffffff');
    let otherId = req.body.otherId;
    let collection = [];
    // console.log(users);
    let user = users.find(item => item.id == otherId);
    // console.log(user,'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
   let col= detailList.detailList.filter(item => {
       console.log(item,'dddddddddddddddddddddddddddddddd');
       if (user.collects.indexOf(item.id) >= 0) {
            return collection.push(item);
        }
    });
    // console.log(col,'jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj');
    if (collection) {
        res.json(col);
    } else {
        res.json({});
